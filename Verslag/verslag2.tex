...
\newpage
\section*{Inleiding}
Files zijn een groot probleem. 
Elke dag weer staan er vele auto's weer in een lange rij op elkaar te wachten.
Dit kost natuurlijk allemaal heel veel tijd en tijd is geld, dus het fileprobleem is een grote kostenpost.
Daarom is het van groot belang dat er een oplossing wordt gezocht voor files.

Alvorens een passende oplossing te vinden moet eerst onderzocht worden hoe files werken.
Daarvoor proberen we antwoord te vinden op de volgende vragen:
Hoe ontstaan files?
Hoe veranderen de files van lengte?
Wat voor soorten files zijn er en wat zijn de verschillen?
Hoe lossen files uiteindelijk weer op?

In dit verslag hebben we geprobeerd antwoord te geven op de eerste en de laatste vraag door het fileprobleem na te simuleren.
Ons hoofddoel hierbij is het vinden van een juiste methode om dit na te bootsen met een numerieke benadering.

\section*{Theorie}
In dit verslag modelleren we de dichtheid van auto's tijdens een file op een snelweg. We beperken ons tot een eendimensionale snelweg van lengte \(L\) (geen afslagen of meerdere wegen die samenkomen). 
...
Om het gedrag van files goed te kunnen begrijpen, hebben we een goede manier bedacht om files na te bootsen.
Door gebruikt te maken van de discretisatie van een weg en te kijken hoe de autodichtheid $\rho$ verandert met de tijd, krijgen we een beeld hoe files kunnen ontstaan en weer kunnen oplossen.
Hierbij hebben we centrale differentie gebruikt bij de benadering van de afgeleides.

We zagen eerst dat het makkelijker is om onze differentiaalvergelijkingen toe te passen op $u = 1-2\rho$, omdat deze er dan iets mooier uitzien.
Bij het onderzoek naar het ontstaan van files vonden we dat de autodichtheid langzaam over de hele weg toeneemt en zich over de weg verspreidt.
Vervolgens hebben we onderzocht wat de maximale tijdstap is zodat de integratie stabiel is: blijkbaar moet voor stabiliteit gelden:
\begin{align*}
\Dt \leq \frac{\Delta x^2}{2\nu} =\frac{L^2}{2\nu(N+1)^2}
\end{align*}

Voor de nabootsing van het oplossen van files hebben we het upwind schema gebruik: een alternatief voor centrale differentie.
Dit om te zorgen dat de dichtheid niet negatief werd.
Hier zagen we dat de auto's langzaamaan de weg verlaten.
Dit gebeurde lineair met de tijd.

\subsection*{De Matlabcode}
